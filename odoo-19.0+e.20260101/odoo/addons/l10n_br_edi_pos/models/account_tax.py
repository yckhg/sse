...
class AccountTax(models.Model):
    ...
    def _prepare_base_line_tax_repartition_grouping_key(self, base_line, base_line_grouping_key, tax_data, tax_rep_data):
        ...
        a value different from $0, and then we need the line so we can modify it to this returned value. We also want
        to record these $0 tax lines for informational purposes. """
        res = super()._prepare_base_line_tax_repartition_grouping_key(base_line, base_line_grouping_key, tax_data, tax_rep_data)
        record = base_line["record"]
        if isinstance(record, models.Model) and record._name == "pos.order.line" and record.order_id.is_tax_computed_externally:
