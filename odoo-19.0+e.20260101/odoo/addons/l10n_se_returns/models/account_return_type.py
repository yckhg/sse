...
class AccountReturnType(models.Model):
    ...
    def _generate_all_returns(self, country_code, main_company, tax_unit=None):
        ...
        ec_sales_return_type = self.env.ref('l10n_se_returns.se_ec_sales_list_return_type')
        months_offset = ec_sales_return_type._get_periodicity_months_delay(main_company)
        previous_period_start, previous_period_end = ec_sales_return_type._get_period_boundaries(
            main_company,
            fields.Date.context_today(self) - relativedelta(months=months_offset),
        )
        company_ids = self.env['account.return'].sudo()._get_company_ids(
            main_company,
            tax_unit,
            ec_sales_return_type.report_id,
        )

        ec_sales_list_tag_ids = [
            *self.env.ref('l10n_se.tax_report_line_35_tag')._get_matching_tags().ids,
            *self.env.ref('l10n_se.tax_report_line_38_tag')._get_matching_tags().ids,
            *self.env.ref('l10n_se.tax_report_line_39_tag')._get_matching_tags().ids,
        ]
