...
class PosPreparationDisplayController(http.Controller):
    ...
    def display_preparation_web(self, display_id=False):
        preparation_display = request.env['pos.prep.display'].search(
            [('id', '=', int(display_id))]
        )
        ...
        context = {
            ...
            'prep_display_id': preparation_display.id,
        }

        response = request.render('pos_enterprise.prep_display_index', context)
        return response
