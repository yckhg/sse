...
WSFE_ERRORS = {
    '501': _lt('This is an internal error in ARCA. Please wait a couple of minutes and try again'),
    '600': _lt('"600: ValidacionDeToken: Error al verificar hash"\n    "600: VerificacionDeHash: Digital signature is not validated". It is usually an error on ARCA servers, to verify this we can go to the type of documents and do a Get Last Invoice, if we get an error then it is due to an ARCA error. It is usually solved within the day.\n * "600: ValidacionDeToken: No apareció CUIT en lista de relacione" It is likely that you has not delegated ARCA services or electronic invoice setup is not completed'),
    '602': _lt('The number or date of the document does not correspond to the next one to be authorized.\n * The invoice date not be after the last invoice validated in ARCA.\n * Verify that the next invoice number in the Journal corresponds with the next one in ARCA\n * If ARCA Concept "Services" or "Products and Services" could be due to the Invoice Date: this one could be null or need to be in the range N-10 and N+10 days being N the date of dispatch of the authorization request.\n\nYou can go to "Consult invoice in ARCA" menu and get the information about the last invoice.'),
    '1006': _lt('It solved using "round globally" method in the company'),
    '10016': _lt("Please note that if you are trying to validate an invoice with a date other than today, you must verify if it falls within the date range according to the ARCA concept or document type:\n a) If it is Product: N+5 or N-5 with N being today's date.\n b) If it is Services or Products and Services: N+10 or N-10 with N being today's date.\n c) If it is a MiPyme Invoice: N-5 or N+1 with N being today's date. For Debit Note or Credit Note only N-5\n\nYou can go to 'Consult invoice in ARCA' menu and get the information about the last invoice.\n"),
    '10016-1': _lt('The invoice date cannot be after the last invoice validated in ARCA.'),
    '10016-2': _lt('There may have been a mismatch in the numbering of this type of document between Odoo and ARCA.'),
    ...
    '10154': _lt('It is possible that you are making a CN/DN but you are setting a wrong value in the "FCE: Is cancellation" field. If the ARCA message indicates:'
    ...
    '10180': _lt('Please check the next conditions:\n    * Review that the invoice total invoice is greater than the amount defined by ARCA for this type of document\n    * Review that the customer is a MiPYME'),
    ...
    '10242': _lt('Recently ARCA changed the types VAT Conditions available. Please verify the ARCA Responsibility in the partner is Deprecated and set a valid one.'),
}

WSFEX_ERRORS = {
    'country_afip_code': _lt('We searched for the destination country of the invoice and had no ARCA code.\n\nPlease enter the ARCA page and look for the code in the System Tables section > Countries found in the bottom right and look for the corresponding code.\n\n Then go to Odoo to the Sales menu > Settings > Address book > Localization > Countries, activate developer mode and add the ARCA code in the corresponding field'),
    '1640': _lt('For invoices of type "(19) FACTURAS DE EXPORTACION" and ARCA Concept "Products / Definitive export of goods" it is require to sent the Incoterm infomation. Please go to Other Info Tab and select the corresponding value in the Incoterm field'),
    '1580': _lt('If the customer belongs to a Free Zone you should give the customer CUIT number in order to continue\n * If is a Foreign Customer is possible that Odoo does not have Legal VAT/Natrual VAT configured for your customer country.\n\n\t 1. Please go to ARCA page and search for the code in the section "Tablas del Sistema > Paises" that is in the bottom right of the page and get the corresponding code for the country.\n\t 2. Go back to  Odoo, in the menú Sales > Settings > Address Book > Localization > Countries, activate the developer mode and add the ARCA code in the corresponding field'),
    '1668': _lt('The registration for the online exportation invoices (invoice generate from ARCA page) is different than the registration for electronic exportation invoices. Therefore you must configure this one also because you will not be able to create invoices of this type in Odoo.\n\nWhen configuring, you must check the option "Factura Electrónica Exportación de Servicios" and then "Webservice/Facturador Plus".\n\nView image https://www.adhoc.com.ar/web/content/162597?unique=202798c78764e1c1d31b35d46556a7947ac511e8&download=true'),
    '2053': _lt('For the invoice case, if you are trying to validate an invoice "ARCA Concept: Service" and the invoice currency is not ARS then the currency rate must be the one of the previous business day of the date where the invoice was issued. If the request has a date previous that the one in the invoice then the rate should be the one of previous business day of the request.\n * For the case of the Debit or Credit Notes of type "Services" the rate should be the same as the related invoice.'),
}

WBSFE_ERRORS = {
    '501': _lt('This is an internal error in ARCA. Contact to your Odoo provider in order to let it know that is happening and to investigate what is going on with the ARCA service'),
...
def _hint_msg(env, error_code: str, afipws: Literal['wsfe', 'wsfex', 'wsbfe']):
    ...
    if afipws == 'wsfe' and error_code in WSFE_ERRORS:
        ...
        return env._('The invoice has not been accepted by ARCA, please fix the errors and try again')
