...
class HrPayslip(models.Model):
    ...
    def _get_representation_fees_threshold(self, localdict):
        ...
        result = 0
        if version.job_id.l10n_be_custom_representation_fees:
            for fee in ['homeworking', 'phone', 'internet', 'car_management']:
                result += self._rule_parameter(f'cp200_representation_fees_{fee}') if version.job_id[f'l10n_be_custom_representation_fees_{fee}'] else 0
        else:
