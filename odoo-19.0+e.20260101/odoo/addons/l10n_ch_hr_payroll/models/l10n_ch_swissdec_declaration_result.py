...
class L10nCHSwissdecJobResult(models.Model):
    ...
    def action_open_status_notification(self):
        ...
        return {
            ...
            'res_model': 'l10n.ch.swissdec.job.result',
            'type': 'ir.actions.act_window',
            "view_id": self.env.ref("l10n_ch_hr_payroll.view_l10n_ch_swissdec_job_result_status_form").id,
            'view_mode': 'form',
            'res_id': self.id,
            "target": "new",
        }
    ...
    def action_get_dialog(self):
        ...
        response = result['soap_response']
        _message = result['request_xml']
        _request_archive = result['response_xml']
    ...
    def _reply_dialog(self, dialog_id):
        ...
        response = result['soap_response']
        _message = result['response_xml']
        _request_archive = result['request_xml']
    ...
    def import_lpp_contributions(self):
        ...
        for person in staff:
            ...
            if contributions and matching_employee and matching_employee.version_id:
                vals = {}
                ...
                if vals:
                    matching_employee.version_id.write(vals)
                    message_log.append(_("LPP Contributions successfully imported in the following contract: %s", matching_employee.version_id._get_html_link()))
        if message_log:
